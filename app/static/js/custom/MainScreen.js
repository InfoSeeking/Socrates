                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                if(fn == 'histogram') {
                                    ...
                                    field = params['input']['x-field'] + '_' + params['input']['y-field'];
                                    console.log(field);
                                    //var field = 'hello_friends'
                                }
                                else if (fn == 'piechart') {
                                    field = params['input']['field'];
                                }
                                else if (fn == 'regression') {
                                    field = params['input']['x-field'] + '&' + params['input']['y-field'];
                                }
                                console.log(field);
                                var card = createResultCard('visualization');
                                card.append("<div class='card-body'><h4 class='card-title'>" + fn_name +"</h4><div><canvas id='" + field + "'></canvas></div></div>")
                                $(".completed.visualization").append(card);

                                //add close button
                                that.getWorkingSet(working_set_id, function(ws) {
                                    TRANS.parseParams(params.input, VIS.specs[mod]["functions"][fn]["param"], ws);
                                    console.log(params);

                                    //var data = ws["data"];

                                    if(fn == 'histogram') {
                                        var data = params['input']['field'];
                                        var index = [];
                                        //var values = [];
                                        for (var i = 0; i<data.length; i++) {
                                            index.push(i);
                                        ...
                                        //create chart
                                        $(document).ready(function() {
                                            var myChart = document.getElementById(field).getContext('2d');
                                            var newChart = new Chart(myChart, {
                                                type: 'bar',
                                        ...
                                        console.log(field);
                                        var dict = [];
                                        for (var i = 0; i<params['input']['x-field'].length; i++) {
                                            var point = {
                                                x: params['input']['x-field'][i],
                                                y: params['input']['y-field'][i]
                                            };
                                            console.log(point);
                                            dict.push(point);
                                        }
                                        ...
                                        var data = [];
                                        var map = {};//map from label to count
                                        var fieldData = params['input']['field'];
