var MainScreen = (function(){
     ...
     */
    that.getDataResults = function(params, Aparams){
        var myData;
        UI.toggleLoader(true);
        //clear cache, since now working set is modified
        UTIL.clearWorkingSetCache();
        ...
        $.ajax({
            ...
            success: function (data, stat, jqXHR) {
                ...
                InterfaceShowResults(myData, myType, null);
    ...
    function getAResults(params){
        var myData;
        UI.toggleLoader(true);
        //clear cache, since now working set is modified
        UTIL.clearWorkingSetCache();
        console.log(UTIL.getCurrentWorkingSetID());
        console.log(params);
        params["working_set_id"] = UTIL.getCurrentWorkingSetID();
        $.ajax({
            url: UTIL.CFG.api_endpoint,
    ...
    //modified showAllData to work with Interface
    //DELETE******
    /*that.interfaceShowAllData = function (data, meta, analysis, typ, index){
        console.log("Showing data for " +data+" "+meta+" "+analysis+" "+ typ + "," + index);
        var aData = null;
        ...
        if(typ == "analysis"){
            ...
            if(index !== null){
             //show only one
             aData = new Array(analysis[index]);
             }
             else{
    ...
    function InterfaceCreateTable(type, data, meta, analysis){
        var section;
        if(type == "collection"){
            ...
            var index = analysis.length - 1;
            var an = analysis[index];
     ...
     */
    that.showResults = function (working_set, type, analysis_index){
    ...
    that.InterfaceShowResults = function (myData, type, analysis_index){
        //var setName = working_set["working_set_name"];
        /*if(showResults.first){
            showResults.first = false;
            $("#workspace #intro").detach();
        ...
        if(type == "collection"){
            ...
            h2.html("Analysis");
            h2.parent().append(closeBoxButton());
            box.append(InterfaceCreateTable(type, myData.data, myData.meta, myData["analysis"]));
            var curIndex = myData["analysis"].length - 1;
            if(analysis_index !== undefined){
                curIndex = analysis_index;
            }
            console.log("Showing " + curIndex);
            if(myData["analysis"][curIndex].hasOwnProperty("entry_meta")){
                //show all data button
                box.append(showAllDataBtn().attr("data-type", "analysis").attr('data-index', curIndex));
                box.append(getDownloadButton().attr("data-type", "analysis").attr('data-index', curIndex));
            }
            ...
            h2.append(" (" + setName + ")");
            h2.parent().append(closeBoxButton());
            var table = InterfaceCreateTable(type, myData, myData.meta, myData.analysis);//this is the HTML created table
            box.append(table);
            box.append(showAllDataBtn().attr("data-type", "collection"));
            box.append(getDownloadButton().attr("data-type", "collection"));
        }
        $("#finalResult").append(box);
        box.hide().fadeIn();
    }
